...
public final class FFMQServer implements FFMQServerMBean, FFMQEngineListener, Runnable {
     ...
     */
    @Override
    public synchronized boolean start(List<AbstractDestinationDefinition> queueList) {
        ...
        try {
            ...
            // Banners
            log.info("FFMQ server " + getVersion() + " (Java " + System.getProperty("java.version") + ")");
            log.info("Running on " + System.getProperty("os.name", "?") + " " + System.getProperty("os.version", "?"));
            log.info("JVM is " + System.getProperty("java.vm.vendor", "?") + " " + System.getProperty("java.runtime.version", "?") + " ["
                            + System.getProperty("java.vm.name", "?") + "] at " + System.getProperty("java.home", "?"));
            ...
            String jmxRmiListenAddr =
                            InetUtils.resolveAutoInterfaceAddress(settings.getStringProperty(FFMQCoreSettings.JMX_AGENT_RMI_LISTEN_ADDR, "0.0.0.0"));
            if (jmxAgentEnabled) {
            ...
            try {
                if (jmxAgent != null) jmxAgent.register(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()), engine);
            } catch (Exception e) {
                log.error("Cannot register local engine on JMX agent", e);
            }
            engine.deploy();
            if (null != queueList && queueList.size() > 0) {
                for (AbstractDestinationDefinition destinationDefinition : queueList) {
                    destinationDefinition.check();
                    if (destinationDefinition instanceof QueueDefinition) {
                        engine.createQueue((QueueDefinition) destinationDefinition);
                    } else {
                        engine.createTopic((TopicDefinition) destinationDefinition);
                    }
                }
            }
            ...
            if (useSSL && useNIOListener) throw new FFMQException("Cannot enable SSL & NIO listener at the same time.", "CONFIGURATION_ERROR"); // JDK
                                                                                                                                                // 1.4
                                                                                                                                                // limitation
    ...
    public synchronized boolean shutdown() {
        ...
        try {
            ...
            engine.undeploy();
            try {
                if (jmxAgent != null) jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()));
            } catch (Exception e) {
                log.error("Cannot unregister local engine from JMX agent", e);
            }

            // Undeploy JMX
            if (jmxAgent != null) jmxAgent.stop();
    ...
    public void engineDeployed() {
        try {
            if (jmxAgent != null) {
                jmxAgent.register(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()
                                + ",children=async-managers,name=notification"), engine.getNotificationAsyncTaskManager());
                jmxAgent.register(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()
                                + ",children=async-managers,name=delivery"), engine.getDeliveryAsyncTaskManager());
                jmxAgent.register(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()
                                + ",children=async-managers,name=disk-io"), engine.getDiskIOAsyncTaskManager());
            }
        } catch (Exception e) {
        ...
        try {
            if (jmxAgent != null) {
                jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()
                                + ",children=async-managers,name=notification"));
                jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()
                                + ",children=async-managers,name=delivery"));
                jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName()
                                + ",children=async-managers,name=disk-io"));
            }
        } catch (Exception e) {
    ...
    public void queueDeployed(LocalQueue queue) {
        try {
            if (jmxAgent != null)
                jmxAgent.register(
                                new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName() + ",children=queues,name="
                                                + queue.getName()), queue);
        } catch (Exception e) {
    ...
    public void queueUndeployed(LocalQueue queue) {
        try {
            if (jmxAgent != null)
                jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName() + ",children=queues,name="
                                + queue.getName()));
        } catch (Exception e) {
    ...
    public void topicDeployed(LocalTopic topic) {
        try {
            if (jmxAgent != null)
                jmxAgent.register(
                                new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName() + ",children=topics,name="
                                                + topic.getName()), topic);
        } catch (Exception e) {
    ...
    public void topicUndeployed(LocalTopic topic) {
        try {
            if (jmxAgent != null)
                jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Engines,engine=" + engine.getName() + ",children=topics,name="
                                + topic.getName()));
        } catch (Exception e) {
    ...
    private void bridgeDeployed(JMSBridge bridge) {
        try {
            if (jmxAgent != null) jmxAgent.register(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Bridges,bridge=" + bridge.getName()), bridge);
        } catch (Exception e) {
    ...
    private void bridgeUndeployed(JMSBridge bridge) {
        try {
            if (jmxAgent != null) jmxAgent.unregister(new ObjectName(JMXAgent.JMX_DOMAIN + ":type=Bridges,bridge=" + bridge.getName()));
        } catch (Exception e) {
