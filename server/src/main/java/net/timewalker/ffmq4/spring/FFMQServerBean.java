...
public class FFMQServerBean {
    ...
    private List<AbstractDestinationDefinition> queueList;
    ...
    public List<AbstractDestinationDefinition> getQueueList() {
        return queueList;
    }

    public void setQueueList(List<AbstractDestinationDefinition> queueList) {
        this.queueList = queueList;
    }

    private void checkProperties() throws JMSException {
        if (engineName == null || engineName.length() == 0) throw new FFMQException("Bean property 'engineName' is required", "INVALID_BEAN_CONFIG");
        if (configLocation == null) throw new FFMQException("Bean property 'configLocation' is required", "INVALID_BEAN_CONFIG");
    }
    ...
    private Settings loadConfig() throws JMSException {
        try {
            ...
            if (configLocation.startsWith("classpath:")) {
                ...
                if (in == null) throw new IllegalArgumentException("Cannot find configuration resource in classpath : " + resourcePath);
                settings.load(in);
                in.close();
            } else {
                File configFile = new File(configLocation);
                if (!configFile.canRead()) throw new IllegalArgumentException("Cannot read configuration file : " + configFile.getAbsolutePath());
